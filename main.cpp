...
void test1()
{
	...
	assert(vm_instance.sp == 10);
	assert(vm_instance.stack[0]->marked);
	assert(vm_instance.stack[1]->marked);
	assert(vm_instance.stack[2]->marked);
	...
	assert(vm_instance.stack[9]->marked);
}
...
void test2()
{
	vm vm_instance;
	{
		...
		vm_instance.del(6);
		vm_instance.gc();
		// only vm_instance[6] should be collect
		assert(vm_instance.currentNumOfObjects == 7);
		assert(vm_instance.sp == 7);

		vm_instance.del(2);
		vm_instance.del(3);
		vm_instance.gc();
		// vm_instance[7] still contains a reference to vm_instance[2] and vm_instance[3], so currentNumOfObjects still 7
		assert(vm_instance.currentNumOfObjects == 7);
		vm_instance.gc();
